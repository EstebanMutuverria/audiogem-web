/**
 * orderPdf.js
 * Premium PDF purchase order builder using jsPDF.
 * Replica fiel del diseño de budgetPdf.js pero orientado a pedidos de
 * compra a proveedor:
 *  – Dark header band with company logo (título "PEDIDO")
 *  – Styled metadata section (Proveedor / Fecha)
 *  – Table with alternating row colors (Producto | Cant.) — SOLO nombres y
 *    cantidades: los precios y totales quedan fuera del PDF a propósito
 *  – Footer with branding
 *
 * Todos los helpers son privados y están duplicados a propósito:
 * no se importa nada de budgetPdf para mantener ambos generadores
 * independientes.
 */
...
// Solo dos columnas: el PDF muestra únicamente nombre y cantidad.
const COL_PRODUCT_W = 140;
const COL_QTY_W = CONTENT_WIDTH - COL_PRODUCT_W;
...
/**
 * Sanitize supplier name for safe filenames.
 */
...
/* ─── Section renderers (privados) ────────────────────────────── */

/**
 * Draw the dark header band with logo, title "PEDIDO", company small text
 * and date below it. Returns the Y coordinate after the header.
 */
const drawHeader = (doc, header, logoData) => {
    ...
    // Title "PEDIDO" (right side)
    doc.setFont('Helvetica', 'bold');
    doc.setFontSize(22);
    doc.setTextColor(...COLORS.headerText);
    doc.text('PEDIDO', PAGE_WIDTH - MARGIN, 20, { align: 'right' });
    ...
    if (header.company) {
        ...
        doc.text(header.company, PAGE_WIDTH - MARGIN, 28, { align: 'right' });
    }
    ...
    if (header.date) {
        ...
        doc.text(header.date, PAGE_WIDTH - MARGIN, 35, { align: 'right' });
    }
...
/**
 * Draw the metadata cards (supplier, date).
 */
const drawMetadata = (doc, header, startY) => {
    let y = startY + 10;

    const fields = [
        { label: 'Proveedor', value: header.supplier },
...
/**
 * Build and save the purchase order PDF document.
 * Solo nombres de productos y cantidades: no se imprime ningún precio.
 * @param {{ header: { company: string, date: string, supplier?: string }, items: Array<{ name: string, quantity: number }> }} payload
 */
export const buildOrderPdf = async (payload) => {
    ...
    // 2. Supplier / date metadata
    const afterMetaY = drawMetadata(doc, header, afterHeaderY);

    // 3. Items table (solo nombre y cantidad)
    drawTable(doc, items, afterMetaY);

    // 4. Footer
    drawFooter(doc);

    // Save
    const filename = header.supplier
        ? `pedido-${sanitizeFilename(header.supplier)}.pdf`
        : 'pedido.pdf';

    doc.save(filename);
};

export default buildOrderPdf;